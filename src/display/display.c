...
// LSB defines on/off (0xA4: normal display, 0xA5: full on)
...
static struct spi_config displayConfig = {.frequency = 10000000,
                                          .operation = SPI_WORD_SIZE_GET(8) |
                                                       SPI_LINES_SINGLE,
                                          .slave = 0,
                                          NULL};
...
int displayInit(void) {
  ...
  cmd = DISPLAY_MIRROR_HORIZONTAL ? DISP_CMD_MIRROR_HORIZ
                                  : DISP_CMD_NO_MIRROR_HORIZ;
  rc = writeCmd(&cmd, sizeof(cmd));
  checkRc(true);

  cmd =
      DISPLAY_MIRROR_VERTICAL ? DISP_CMD_MIRROR_VERT : DISP_CMD_NO_MIRROR_VERT;
  rc = writeCmd(&cmd, sizeof(cmd));
  checkRc(true);
...
int displayMemset(uint8_t value, size_t len) {
  ...
  struct spi_buf spiBuf = {.buf = NULL, .len = len};
  struct spi_buf_set spiBufSet = {.buffers = &spiBuf, .count = 1};
...
static int writeCmd(uint8_t *cmd, size_t len) {
  ...
  struct spi_buf spiBuf = {.buf = cmd, .len = len};
  struct spi_buf_set spiBufSet = {.buffers = &spiBuf, .count = 1};
