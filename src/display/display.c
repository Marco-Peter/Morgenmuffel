...
#if USE_OS
#define dispDelay(x) vTaskDelay(x)
#else
#define dispDelay(x) HAL_Delay(x)
#endif
...
#define DISP_MIRROR_VERT 0xA0 // Set segment remap (0xA0: normal, 0xA1: vertical mirror)
#define DISP_MUX_RATIO 0xA8 // Set multiplex ratio \
    // 2nd byte: Multiplex ration(16..64, Mux ratio = N + 1)

#define DISP_MIRROR_HORIZ 0xC0 // Mirror horizontally (0xC0: normal, 0xC8: mirrored)
 ...
 *
 *****************************************************************************/
static int writeCmd(Display* pInst, uint8_t* cmd, uint16_t len)
{
    int res;
