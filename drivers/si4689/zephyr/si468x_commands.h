...
#define SI468X_RPL_GET_DIGITAL_SERVICE_LIST_HDR			2 // contains just the number of bytes
...
typedef struct SI468xDabDigradStatus {
	...
	uint8_t
		rssiLInt:1,			// RSSI below DAB_DIGRAD_RSSI_LOW_THRESHOLD
...
typedef struct SI468xDabEventStatus {
	// Answer byte 0 (from LSB to MSB)
	uint8_t
		svrListInt:1,	// New service list version available
	...
	uint8_t
		svrList:1,		// Service list available --> dabGetDigitalServiceList()
...
typedef struct SI468xServiceComponent {
	...
	uint8_t
		isCa:1,			// access control applies on component
	...
	uint8_t
		uaInfoValid:1,	// User application information is valid
...
typedef struct SI468xDigitalService {
	...
	uint8_t
		pdFlag:1,		// 0: Program (audio); 1: Data service
	...
	uint8_t
		nComponents:4,	// number of available service components
	...
	uint8_t
		siCharset:4,	// used character set for this service --> ETSI TS 101 756, clause 5.3
...
SI468xErr si468xReadOffset(SI468x *pInst, uint16_t offset, uint16_t nBytes, uint8_t *buffer);
...
SI468xErr si468xDabGetFreqList(SI468x *pInst, uint8_t *nFreqs, uint32_t *buffer, uint8_t maxFreqs);
SI468xErr si468xDabTuneFreq(SI468x *pInst, uint8_t freqIndex, uint16_t antCap);
SI468xErr si468xDabDigradStatus(SI468x *pInst, uint8_t digradAck, uint8_t stcAck,
		uint8_t atTune, SI468xDabDigradStatus *status);
SI468xErr si468xDabGetEventStatus(
		SI468x *pInst, uint8_t digradAck, SI468xDabEventStatus *evtStatus);
SI468xErr si468xDabGetDigitalServiceList(
		SI468x *pInst, SI468xDigitalServiceList *serviceList, uint16_t maxSize);
SI468xErr si468xDabStartDigitalService(SI468x *pInst, uint8_t startStop, uint32_t serviceId, uint16_t compId);
SI468xErr si468xGetDigitalServiceData(
		SI468x *pInst, uint8_t statusOnly, ServiceData *serviceData);
SI468xErr si468xDabGetTime(SI468x *pInst, SI468xDateTime *dateTime, uint8_t local);


// Helper functions for service list access
SI468xDigitalService *si468xDabGetService(SI468xDigitalServiceList *svcList, uint8_t svcNum);
SI468xServiceComponent *si468xDabGetServiceComp(SI468xDigitalService *service, uint8_t compNum);

SI468xErr si468xFlashSetPropList (SI468x *pInst, uint8_t *propList, uint16_t size);
SI468xErr si468xFlashEraseChip (SI468x *pInst);
SI468xErr si468xFlashProgramImage (SI468x *pInst, uint32_t addr, uint8_t *buffer, uint16_t size);

#define si468xAnyInt(_pInst_) \
		((_pInst_)->status.stcInt || (_pInst_)->status.acfInt || (_pInst_)->status.rdsInt || \
		(_pInst_)->status.rsqInt || (_pInst_)->status.dacqInt || (_pInst_)->status.devntInt)
