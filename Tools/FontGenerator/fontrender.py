...
class FontRender:
	...
	def getPixelValue(self, x, y):
		...
		if x >= self.width():
			raise FontRenderError(str("x value {:d} exceeds glyph width".format(x)))
		if y >= self.size:
			raise FontRenderError(str("y value {:d} exceeds glyph height".format(y)))
		...
		if (y >= rowOffset) and (y < rowOffset + self.height()):
			...
			try:
				...
				print("buffer length: {}").format(len(self.face.glyph.bitmap.buffer))
				raise
		else:
			return False
	...
	def transformGlyph(self):
		...
		output = bytearray(self.face.glyph.bitmap.width * pages * PAGESIZE // 8)
		...
		for column in range(self.face.glyph.bitmap.width):
			...
			for page in range(pages):
				output[self.bitmapWidth() * page * PAGESIZE // 8 + column] = colByte & 0xFF
				colByte = colByte >> PAGESIZE
		return output

if __name__ == '__main__':
	if len(sys.argv) < 4:
		print("Usage: python fontrender.py <fontname> <fontsize> <characterfile>")
		sys.exit()
	...
	outFile = codecs.open("font_" + fr.familyName() + "_" + sys.argv[2] + ".c", 'w', "UTF-8")
